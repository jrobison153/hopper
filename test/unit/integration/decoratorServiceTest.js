...
describe('Decorator Service Tests', () => {
  ...
  describe('when sending begin processing notification', () => {
    ...
    beforeEach(() => {
      ...
      lastPublishedEvent = JSON.parse(redisClientSpy.lastPublishedEvent);
    });

    it('publishes the BATCH_TICKER_PROCESSING_STARTED event', () => {

      expect(lastPublishedEvent.name).to.equal('BATCH_TICKER_PROCESSING_STARTED');
    });

    it('publishes the event with the id in the payload', () => {

      expect(lastPublishedEvent.payload.id).to.equal('an id of some kind');
    });
